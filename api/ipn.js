...
export default async function handler(req, res) {
  ...
  if (!process.env.REDIS_URL) return res.status(500).json({ error: 'REDIS_URL not configured' });
  if (!process.env.PESAPAL_CONSUMER_KEY || !process.env.PESAPAL_CONSUMER_SECRET) return res.status(500).json({ error: 'Pesapal keys not configured' });
