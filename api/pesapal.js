...
async function createOrder(token, ipnId, { plan, amount, email, phone, firstName, lastName, orderId }) {
  ...
  const body = {
    ...
    callback_url: `${HOST}/success?orderId=${safeOrderId}&plan=${planCode}`,
    ...
    if (req.method === 'POST' && action === 'create') {
      ...
      if (!result.redirect_url) {
        return res.status(400).json({
          error: (typeof result.error === 'string' ? result.error : (result.error?.message || result.message)) || 'Order creation failed',
          detail: result
        });
      }
