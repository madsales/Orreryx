...
const HOST    = (process.env.PESAPAL_HOST || 'https://orreryx.vercel.app').replace(/\/$/, '');
...
async function createOrder(token, ipnId, { plan, amount, email, phone, firstName, lastName, orderId }) {
  ...
  const body = {
    ...
    callback_url: `${HOST}/success?orderId=${safeOrderId}&order=${safeOrderId}&plan=${planCode}`,
...
export default async function handler(req, res) {
  ...
  if (!KEY || !SECRET) {
    return res.status(500).json({ error: 'Pesapal keys not configured. Set PESAPAL_CONSUMER_KEY and PESAPAL_CONSUMER_SECRET in Vercel.' });
  }
  if (!HOST || !/^https?:\/\//.test(HOST)) {
    return res.status(500).json({ error: 'PESAPAL_HOST must be set to your full site URL, for example https://orreryx.io' });
  }
