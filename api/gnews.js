// api/gnews.js — Google News RSS live feed
// Fetches 3 topic streams from Google News RSS → deduplicates → returns up to 60 articles

const CACHE_TTL = 10 * 60 * 1000; // 10-minute cache
let cache = null, cacheTime = 0;

const FEEDS = [
  'https://news.google.com/rss/search?q=war+military+conflict+attack+missile&hl=en-US&gl=US&ceid=US:en',
  'https://news.google.com/rss/search?q=economy+sanctions+trade+tariff+inflation&hl=en-US&gl=US&ceid=US:en',
  'https://news.google.com/rss/search?q=geopolitics+diplomacy+nuclear+cybersecurity&hl=en-US&gl=US&ceid=US:en',
];

function categorize(title) {
  const t = (title || '').toLowerCase();
  if (/nuclear|nuke|uranium|warhead|atomic|iaea/.test(t))            return { cat:'nuc', catLabel:'NUCLEAR',     severity:'critical' };
  if (/cyber|hack|ransomware|malware|data.breach|ddos/.test(t))      return { cat:'cyb', catLabel:'CYBER',       severity:'high'     };
  if (/\bai\b|artificial intelligence|chatgpt|openai|semiconductor/.test(t)) return { cat:'tech', catLabel:'TECHNOLOGY', severity:'low' };
  if (/earthquake|tsunami|hurricane|typhoon|flood|wildfire|volcano/.test(t)) return { cat:'dis', catLabel:'DISASTER',    severity:'high' };
  if (/climate|carbon|emissions|cop\d+|deforestation/.test(t))       return { cat:'env', catLabel:'ENVIRONMENT', severity:'medium'   };
  if (/disease|virus|pandemic|outbreak|vaccine|who.declares/.test(t))return { cat:'hlt', catLabel:'HEALTH',      severity:'medium'   };
  if (/\boil\b|\bgas\b|opec|pipeline|petroleum|energy/.test(t))      return { cat:'nrg', catLabel:'ENERGY',      severity:'medium'   };
  if (/crime|murder|arrest|corruption|terrorist|cartel/.test(t))     return { cat:'cri', catLabel:'CRIME',       severity:'medium'   };
  if (/ceasefire|peace.talks|treaty|sanction|summit|diplomat/.test(t))return { cat:'dip', catLabel:'DIPLOMATIC', severity:'medium'   };
  if (/economy|inflation|recession|gdp|trade.war|tariff|central.bank/.test(t)) return { cat:'eco', catLabel:'ECONOMIC', severity:'medium' };
  if (/company|merger|ipo|earnings|billion|ceo|startup|layoff/.test(t)) return { cat:'biz', catLabel:'BUSINESS', severity:'low'      };
  if (/war|attack|bomb|missile|military|army|troops|airstrike/.test(t)) return { cat:'mil', catLabel:'MILITARY', severity:'high'     };
  if (/election|president|parliament|minister|government|vote|protest/.test(t)) return { cat:'pol', catLabel:'POLITICS', severity:'low' };
  return { cat:'pol', catLabel:'POLITICS', severity:'low' };
}

function parseRss(xml, idOffset) {
  const items = [];
  const itemRe = /<item>([\s\S]*?)<\/item>/g;
  let m, i = 0;
  while ((m = itemRe.exec(xml)) !== null && i < 20) {
    const b = m[1];
    const rawTitle  = (b.match(/<title>(?:<!\[CDATA\[)?([\s\S]*?)(?:\]\]>)?<\/title>/)      || [])[1] || '';
    const rawLink   = (b.match(/<link>([\s\S]*?)<\/link>/)                                   ||
                       b.match(/<guid[^>]*isPermaLink[^>]*>([\s\S]*?)<\/guid>/)               ||
                       b.match(/<guid>([\s\S]*?)<\/guid>/)                                    || [])[1] || '';
    const rawDate   = (b.match(/<pubDate>([\s\S]*?)<\/pubDate>/)                             || [])[1] || '';
    const rawSource = (b.match(/<source[^>]*>([\s\S]*?)<\/source>/)                          || [])[1] || 'Google News';

    // Strip " - Source Name" suffix Google appends to titles
    const title = rawTitle
      .replace(/&amp;/g,'&').replace(/&lt;/g,'<').replace(/&gt;/g,'>').replace(/&#39;/g,"'").replace(/&quot;/g,'"')
      .replace(/\s+-\s+[^-]{2,40}$/, '')
      .trim();
    if (!title || title.length < 10) continue;

    const pubDate = rawDate ? new Date(rawDate).toISOString() : new Date().toISOString();
    const { cat, catLabel, severity } = categorize(title);

    items.push({
      id:       7000 + idOffset + i,
      cat, catLabel, severity,
      lat:      null,
      lng:      null,
      loc:      rawSource.trim(),
      txt:      title,
      url:      rawLink.trim(),
      source:   rawSource.trim(),
      tags:     [],
      time:     pubDate,
      fromGNews: true,
    });
    i++;
  }
  return items;
}

function dedupByTitle(events) {
  const seen = new Set();
  return events.filter(e => {
    const key = e.txt.toLowerCase().replace(/\s+/g,' ').substring(0, 70);
    if (seen.has(key)) return false;
    seen.add(key);
    return true;
  });
}

export default async function handler(req, res) {
  res.setHeader('Access-Control-Allow-Origin', '*');
  res.setHeader('Access-Control-Allow-Methods', 'GET,OPTIONS');
  res.setHeader('Access-Control-Allow-Headers', 'Content-Type');
  res.setHeader('Cache-Control', 'public, max-age=600, stale-while-revalidate=60');
  if (req.method === 'OPTIONS') return res.status(200).end();

  if (cache && Date.now() - cacheTime < CACHE_TTL) {
    return res.status(200).json(cache);
  }

  try {
    const results = await Promise.allSettled(
      FEEDS.map((url, i) =>
        fetch(url, {
          headers: { 'User-Agent': 'OrreryIntelligence/1.0 (https://www.orreryx.io)' },
          signal:  AbortSignal.timeout(8000),
        })
          .then(r => r.ok ? r.text() : Promise.reject(new Error(`${r.status}`)))
          .then(xml => parseRss(xml, i * 100))
          .catch(() => [])
      )
    );

    let all = [];
    results.forEach(r => { if (r.status === 'fulfilled') all = all.concat(r.value); });

    all.sort((a, b) => new Date(b.time) - new Date(a.time));
    const final = dedupByTitle(all).slice(0, 60);

    cache = { articles: final, fetched: Date.now(), count: final.length, source: 'gnews' };
    cacheTime = Date.now();

    console.log(`[GNews] ${final.length} articles fetched`);
    return res.status(200).json(cache);

  } catch (err) {
    console.error('[GNews] Error:', err.message);
    return res.status(200).json({ articles: [], error: err.message, source: 'gnews', fetched: Date.now() });
  }
}

export const config = { api: { bodyParser: false } };
