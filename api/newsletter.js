...
const FROM    = process.env.EMAIL_FROM || 'OrreryX Intel <onboarding@resend.dev>';
...
function emailTemplates(email) {
  ...
  const footer = `<div style="margin-top:32px;padding-top:20px;border-top:1px solid rgba(255,255,255,.08);font-size:11px;color:#484844">© 2026 OrreryX · <a href="${HOST}" style="color:#484844">orreryx.io</a> · <a href="${unsub}" style="color:#484844">Unsubscribe</a></div>`;
  const wrap = (content) => `<div style="background:#09090b;color:#f0f0ec;padding:40px;max-width:520px;margin:0 auto;border:1px solid rgba(255,255,255,.1);border-radius:8px;font-family:'Helvetica Neue',sans-serif"><div style="margin-bottom:24px;font-size:16px;font-weight:800">⊕ OrreryX</div>${content}${footer}</div>`;

  return {
    // Day 0 — Welcome (sent immediately on signup)
    welcome: {
      subject: 'Welcome to OrreryX Intel — Your geopolitical edge starts now',
      html: wrap(`
    ...
    day1: {
      ...
      html: wrap(`
        ...
        <div style="font-size:13px;color:#a0a09a;margin-bottom:24px">You subscribed to OrreryX yesterday. Here's the intelligence briefing you asked for.</div>
    ...
    day3: {
      subject: 'How a London macro fund uses OrreryX (3-minute read)',
      html: wrap(`
        <div style="font-size:20px;font-weight:800;margin-bottom:12px">How Professional Traders Use Geopolitical Intelligence</div>
        <div style="font-size:13px;color:#a0a09a;margin-bottom:20px">You signed up to OrreryX Intel 3 days ago. Here's how our most active users make money with it.</div>
        <div style="background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.08);border-radius:8px;padding:20px;margin-bottom:20px">
          ...
          <div style="font-size:13px;color:#c0c0b8;line-height:1.8">
            <strong style="color:#f0f0ec">6:45am:</strong> Check OrreryX dashboard. Iran risk score jumped from 72 → 87 overnight.<br>
    ...
    day30: {
      ...
      html: wrap(`
        ...
        <div style="font-size:13px;color:#a0a09a;margin-bottom:20px">You joined OrreryX Intel a month ago. The geopolitical landscape has shifted significantly. Here's what's changed:</div>
        <div style="background:rgba(244,67,54,.08);border-radius:8px;padding:20px;margin-bottom:20px">
          <div style="font-size:13px;color:#c0c0b8;line-height:1.9">
            ...
            🔴 <strong style="color:#f0f0ec">New conflicts:</strong> 3 new flashpoints added to OrreryX tracker<br>
...
async function broadcastPush() {
  ...
  const payload = JSON.stringify({
    title: `☢ OrreryX Risk Brief — ${today}`,
